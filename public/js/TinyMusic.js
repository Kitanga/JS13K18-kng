(function (root, factory) {
  ...
  function Sequence(ac, tempo, arr) {
    ...
    this.pulseWidth = this.ac.createGain();
  }

  // create gain and EQ nodes, then connect 'em
  Sequence.prototype.createFxNodes = function (gain) {
    var eq = [
        ['bass', 100],
        ['mid', 1000],
        ['treble', 2500]
      ],
      prev = this.gain = gain || this.ac.createGain();
  ...
  Sequence.prototype.createPulseOscillator = function () {
    ...
    var constantShaper = this.ac.createWaveShaper();
    constantShaper.curve = constantCurve;
    node.connect(constantShaper);
    constantShaper.connect(widthGain);
    ...
    node._disconnect = node.disconnect.bind(node);
  ...
  Sequence.prototype.stop = function () {
    if (this.osc) {
      ...
      if (this.osc._disconnect) {
        this.osc._disconnect();
      }
      this.osc.disconnect();
      this.osc = null;
    }
